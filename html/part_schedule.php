...
// isset() add 20110621 by nanbuwks
	if (isset($d) && is_numeric($y) && is_numeric($m) && is_numeric($d) && checkdate($m,$d,$y)) {
	...
	print "<TD WIDTH=15% NOWRAP ALIGN=RIGHT>";
        // modify by nanbuwks 20110621
	print "<A HREF=\"./schedule/add/?p=top&y=".date("Y",$firststamp)."&m=".date("n",$firststamp)."&d=".date("j",$firststamp)."\"><IMG SRC=\"./image/entrysadd.gif\" WIDTH=10 HEIGHT=9 BORDER=0 ALT=\"新規予定\"></A>";
	print "</TD>";
	print "</TR>";
	...
	while(list($user_id,$user_name)=each($users)) {
		...
		$text = "";
		$code = "";
		if ($user_id==$login_id && isset($_GET["location"]) && !empty($_GET["location"])) {
			$code = $_GET["location"];
			...
			// modify by nanbuwks 20110621
			   ...
			   if (sizeof($columns)>0 && is_array($columns)) {
				...
				while (list($seqno,$val)=each($columns)) {
                                        // modify by nanbuwks 20111130,20170324
					// $sql = "SELECT * FROM schedules WHERE seqno=".$seqno;
		                        // $sql  = "SELECT schedules.seqno AS seqno ,datefrom,dateto,timefrom,timeto,user_id,create_user_id,viewsign,opensign,note,value FROM schedules,schedule_kind WHERE schedules.seqno='".$seqno."' and  to_number(schedules.kind,'99999') = schedule_kind.key ";
