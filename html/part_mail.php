...
if ($login) {
	...
	// 伝言(受信)
	$sql = "SELECT *,mail_res.res_seqno FROM mail LEFT JOIN (SELECT seqno as res_seqno FROM mail_res WHERE user_id='$login_id') AS mail_res ON mail.seqno = mail_res.res_seqno WHERE user_id='$login_id' AND \"from\" <> '$login_id' AND res_seqno IS NULL ORDER BY createstamp DESC,seqno DESC LIMIT 15";
	...
	print "<TD NOWRAP><A HREF=\"./mail/\">伝言メモ</A> ";
	if ($cnt>0) {
		print "<FONT COLOR=#6666FF>(未読".number_format($cnt)."件)</FONT>\n";
	}
	print "</TD>";
	...
	if ($cnt==0) {
		...
		for($i=0;$i<$cnt;$i++){
			...
			print "●</FONT>&nbsp;";
#			print "<A HREF=\"./mail/view.phtml?n=".$row["seqno"]."\">";
			print "<A HREF=\"./mail/add/?n=".$row["seqno"]."\">";
...
#			print " <FONT COLOR=#666666>".date("n/j H:i",datetime2timestamp($row["sendstamp"]))."<BR>";
			print " <FONT COLOR=#666666>".date("n/j H:i",datetime2timestamp($row["createstamp"]))."<BR>";
