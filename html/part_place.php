...
if ($login) {
	...
	## 在席情報の更新(ロギングしてるのでうまく使えば工程管理の実績収集なんかにつかえるかも)
	if (isset($_GET["location"]) && !empty($_GET["location"])) {
		## 60日間以前のログを削除
		pg_query($conn,"DELETE FROM users_location WHERE updatestamp+'".(24*60).":00'<now()");
		...
		if ($lastcode!=$code && !empty($text)) {
			$sql = "INSERT INTO users_location (seqno,user_id,code,text,updatestamp) VALUES (";
			$sql.= db_textsafe($seqno).",";
			$sql.= "'".db_textsafe($login_id)."',";
			$sql.= "'".db_textsafe($code)."',";
	...
	while(list($user_id,$user_name)=each($users)) {
		...
		if ($user_id==$login_id) {
			...
			if ($code=="1") {
				print "<INPUT TYPE=BUTTON STYLE=\"font-size:80%\" VALUE=\"不在\" ONCLICK=\"location.href='./?location=2'\">";
			} else {
				print "<INPUT TYPE=BUTTON STYLE=\"font-size:80%\" VALUE=\"在席\" ONCLICK=\"location.href='./?location=1'\">";
			}
			print "</TD>\n";
			print "</FORM>\n";
		} else {
