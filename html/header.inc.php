<?php
//      $headerr[] = $_POST["userid"].$_POST["mode"]."ご利用のブンいかてださい";
	if (isset($mode) && $mode=="login") {
		session_unset();
// 		session_id(str_replace("-","",str_replace("_","",$userid)));
 	}
  session_start();
...
#      if ($_COOKIE["PHPSESSID"] != $_POST["sid"]) {
      ...
      if ($userid=="") {
        ...
        if ($pwd=="") {
          ...
          $sql .= "WHERE id='".textsafe($userid)."' AND (passwd='".textsafe($pwd_cr)."' or passwd='".textsafe($pwd_c)."')";
#          $sql .= "WHERE id='".$userid."' AND passwd='".$pwd_cr."'";
          ...
          if ($cnt==0) {
            ...
            header("Set-Cookie: pgo=".urlencode($login_str)."; path="."/;");

            $sql  = "UPDATE users SET session_id='".textsafe($sid)."', session_laststamp='now()' ";
            $sql .= "where id='".textsafe($_POST["userid"])."'";
            pg_query($conn, $sql);
  ...
  case "dlogin":
    // ダイレクトログイン
    if ($sn != "") {
      $sql = "SELECT * FROM users WHERE onetime_passwd='".textsafe($sn)."'";
      $res = pg_query($conn,$sql);
      $cnt = pg_num_rows($res);
      if ($cnt>0) {
        $row = pg_fetch_array($res,0);
        ...
        $sql = "UPDATE users SET session_id='".textsafe($sid)."', onetime_passwd='', session_laststamp='now()' WHERE id='".textsafe($row["id"])."'";
        pg_query($conn, $sql);
      }
    }
    break;
  ...
  case "logout":
    ...
    if (!empty($_COOKIE["pgo"])) {
      ...
      // ユーザー存在確認
      $sql = "SELECT * FROM users WHERE session_id='".textsafe($sid)."'";
      $res = pg_query($conn,$sql);
      $cnt = pg_num_rows($res);
      if ($cnt>0) {
        $sql = "UPDATE users SET session_id=NULL where session_id='".textsafe($sid)."'";
        pg_query($conn,$sql);
      }
    }
    $headertext .= "Set-Cookie: ".session_name()." expires=".gmdate("D, d M Y H:i:s",time()-3600)." GMT; path="."/;\n";
    header("Set-Cookie: ".session_name()." expires=".gmdate("D, d M Y H:i:s",time()-3600)." GMT; path="."/;\n");
    $_SESSION = array(); 
    session_unset();
    session_destroy();
    ...
    header("Set-Cookie: pgo=; expires=".gmdate("D, d M Y H:i:s",time()-3600)." GMT; path="."/;\n",false);
    $headertext .= "Location: ".$access.$domain."/?logout_now=true;\n";
    header("Location: ".$access.$domain."/?logout_now=true;\n",false);
    break;
    ...
    if (!empty($_COOKIE["pgo"])) {
      ...
      if ($cnt==1) {
        ...
        if (crypt($row["id"],$sid)==$login_str) {
          ...
          // modify nanbuwks 20170325
          header("Set-Cookie: pgo=".urlencode($login_str)."; path=".$toppath."/;\n" , false);
          $login_id = $row["id"];
        ...
        $headerr[] = $sql." ".textsafe($sid)."同じユーザーIDで他PCからのログインがあったため自動的にログアウトしました。";
        $login = true;  // false to true 20140509 nanbuwks
        $headertext .= "Set-Cookie: pgo=; expires=".gmdate("D, d M Y H:i:s",time()-3600)." GMT; path="."/;\n";
      }
    } else {
      $login = false;
  ...
  $download_sign = "f";
  if ($login) {
    $sql = "select * from users where id='".textsafe($login_id)."'";
    $res = pg_query($conn,$sql);
    $cnt = pg_num_rows($res);
    if ($cnt>0) {
      $row = pg_fetch_array($res,0);
...
// modify by nanbuwks 20170324,20170325
//  header($headertext);
  $header_do=preg_split("~\n~", "$headertext");
  foreach( $header_do as $header_value ){
         header($header_value,false);
  }
	if ($nowlogin) {
		header("Location:".getenv("SCRIPT_NAME"),false);
		exit;
	}
...
<HEAD>
<!--・-->
<TITLE>ADELIE OFFICE</TITLE>
<?php // modify nanbuwks 20170324
...
#	if ($nowlogin) {
#		print "<HTML><HEAD>\n";
#		print "<META http-equiv=\"refresh\" content=\"0; URL=".getenv("SCRIPT_NAME")."\">\n";
#		print "</HEAD></HTML>\n";
#		exit;
#	}
	if ($login) {
		print "<META http-equiv=\"page-exit\" content=\"BlendTrans(Duration=0.2)\">\n";
	}
?>
<STYLE TYPE="text/css">
<?php
...
//      $d = $tempdate["mday"];
    ...
    if ($print != "on") {
      ...
      if ($admin_sign=="t") {
        print "<A HREF=\"$toppath/admin/setting/\"><img src=\"$toppath/image/btn_admin.gif\" border=0 WIDTH=62 HEIGHT=21 ALIGN=ABSMIDDLE ALT=\"管理者設定へ\"></A> ";
      }
...
<TABLE><FORM NAME="login" ACTION="<?php echo $toppath; ?>/index.phtml" METHOD="POST">
<INPUT TYPE=HIDDEN NAME="mode" VALUE="login">
<TR><TD>ログインID：</TD><TD><INPUT TYPE=TEXT MAXLENGTH=60 SIZE=26 NAME="userid" VALUE="<?php echo $userid; ?>" STYLE="width:100px;ime-mode:disabled">&nbsp;</TD></TR>
